...
var Parser = function() {
	...
	types = {
		...
		"Field" : {
			...
			description : {
				...
				// disabled
				type : 'text', // {'text' | 'password' | 'file' | 'radio'}
	...
	// auxiliary function 1 - check for available variables in types properties,
	// get their value from default arguments (defArgs)
	...
	function addEvents(component, description) {
		...
		if (!!description.action) {
			...
			for (event in description.action) {
				// console.log("event", [event, description.action[event],
				// component]);
				// if component handle addListener method and dispatcher allow
				// to use this event
	...
	function apply(type, specificDescription) {
		...
		/*
		 * // apply private properties if(!!types[type].privateDescription){ obj =
		 * Ext.apply(obj, types[type].privateDescription); }
		 */
		return obj;
	}
