...
def make_map(config):
    ...
    map.connect('/menu/shortDescription/{id}', controller='menu', action='shortDescription')
    map.connect('/dishes/{action}/{group}/{dishId}', controller = 'dishes')
    map.connect('/dishes/{action}/{dishId}', controller = 'dishes')
