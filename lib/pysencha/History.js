var History = function(window) {
	...
	function init(){
		...
		// set current token
		//currentToken.token = fcleanToken(window.location.hash);
	...
	function pushState(shortDescription, title, url, keepHash){
		...
		if(!keepHash){
			//console.log("zmieniam hasha na "+currentToken.token);
			window.location.hash = currentToken.token;
		}
		//console.log("push state: currentToken, statesHistory", [fgetCurrentToken(), statesHistory]);
		
		// delete all history records after previous
		if(currentToken.id++ !== statesHistory.length){
			statesHistory.splice(currentToken.id);
		}
		//console.log("states removed", [statesHistory]);
		
		// add state to history
		statesHistory.push({shortDescription: shortDescription, title: title, url: url});
		//console.log("states updated", [statesHistory]);
	...
	function onHashChange(obj){
		...
		//console.log("before: currentToken, statesHistory, currentState", [currentToken, statesHistory, currentState]);
		
		// callback after back or forward button
		if(recentlyAdded !== true){
			//console.log("pewnie jakiś back");
			
			// get encoded token from newURL property
			
			token = cleanToken(getHash(obj.newURL) || Dispatcher.Config().defaultURL);
			...
			// newURL was not in states history - call Dispatcher action with new url
			...
			if(i === -1){
				//console.log("nie znalazłem w histori - dodaje");
				Dispatcher.action(decodeURI(token), true);
			...
			// newURL was in states history - update currentToken
				...
				//console.log("znalazłem:", [i, currentToken]);
			}
		}
