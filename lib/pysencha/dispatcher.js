var Dispatcher = function()
{
	...
	var screen = {items: {items: []}, dockedItems: {items: []}}, page;
	...
	function fgetFromURL(url){
		...
		return {id : url, items: [], type: "Panel", dockedItems: []};		// just to test
	};
	...
	function fanalyzeDepth(ar, sd){
		...
		*/
		...
		items[0] = ar["items"]["items"];				// reference to items
		itemsCopy[0] = items[0].slice();	// copy of items
		
		items[1] = ar["dockedItems"]["items"];		// reference to dockedItems
		...
		for(i = 0, length = slots.length; i < length; i++){
			...
			if(wantedItem === false){
				...
				description = Parser.transform(description);	// parse description to sencha format
			}
			else{
