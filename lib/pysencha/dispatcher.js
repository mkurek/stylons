var Dispatcher = function() {
	...
	// cut from here
	/*
	 * function farrayFind(items, slot, id) { var i, length, item;
	 * 
	 * for (i = 0, length = items.length; i < length; i++) { if (items[i].slot
	 * === slot && items[i].id === id) { item = items.splice(i, 1); return
	 * item[0]; } } return false; };
	 */
	// cut to here
	/**
	...
	/**
	 * @private Check if items of actual level in screen match actual level in
	 *          short description
	 * 
	 * @param {object}
	...
	function userRequest(url, keepHash) {
		...
		// console.log("action", [url, keepHash]);
	...
	function pageReload(state, newPage) {
		...
		console.log("in page reload: ", [state, newPage]);
		
		/*
		 * fanalyzeDepth(page, shortDescription); // finit();
		 * SenchaAdapter.refresh(page, !!newPage); // page.doLayout();
		 * console.log("After adding:"); console.log(page);
		 */
	};
